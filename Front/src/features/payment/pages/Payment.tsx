...
interface PaymentOutletContext {
  setActivePayment: React.Dispatch<React.SetStateAction<boolean>>;
}
...
const Payment = () => {
  const { setActivePayment } = useOutletContext<PaymentOutletContext>();
  ...
  return (
    <div>
      ...
      <div className="lg:hidden text-sm text-gray-400 mb-2">
        ...
        <span className="text-primary">Payment&amp;Instalments</span>
      </div>

      <div>
        <h3 className="font-semibold">Cards</h3>
        <span className="text-gray-400">manage payment methods</span>
      </div>

      <div className="mt-[2.5rem] flex flex-col gap-[2rem]">
        <div className="flex flex-col lg:flex-row lg:items-center gap-3">
          <div className="bg-gray-50 h-[4rem] w-full lg:w-[20rem] flex items-center justify-between px-[1.5rem] rounded-[0.5rem]">
            <span className="text-gray-400">Credit or Debit cards</span>
            <button onClick={() => setActivePayment(true)}>
              <EditIcon />
            </button>
          </div>
      ...
      <div className="mt-[2rem] pt-[1.5rem] border-t lg:border-t-0 lg:pt-0">
        <span className="font-semibold">Instalments</span>
        <NavLink
          to="/user/payment/instalment"
          className="flex items-center gap-3 mt-[1rem]"
        >
          <span className="text-primary-300">Manage your instalment</span>
          <svg
            width="20"
            height="20"
            viewBox="0 0 24 24"
